export const datosPrueba = [
    {
    id: "1",
    formPlato: "Spaghetti a la boloñesa",
    formDuracion: 45,
    formPorciones: 4,
    formImagen: "https://images.pexels.com/photos/1279330/pexels-photo-1279330.jpeg",
    formDificultad: "Media",
    ingredientes: [
      "300 g de carne picada de res",
      "200 g de spaghetti",
      "1 lata de tomate triturado",
      "1 cebolla picada",
      "1 diente de ajo",
      "Aceite de oliva",
      "Sal y pimienta al gusto"
    ],
    pasos: [
      "Cocina los spaghetti según las instrucciones del paquete.",
      "En una sartén, sofríe la cebolla y el ajo con aceite de oliva.",
      "Agrega la carne y cocina hasta dorar.",
      "Añade el tomate y cocina por 15 minutos.",
      "Sirve los spaghetti con la salsa por encima."
    ],
    formTip: "Agrega un poco de vino tinto para realzar el sabor.",
    formDescripcionBreve: "Pasta clásica italiana con carne y salsa de tomate.",
    formDescripcionAmplia: "Una receta tradicional y reconfortante de la cocina italiana. Ideal para compartir en familia. Los sabores de la carne y el tomate se mezclan a la perfección con la textura al dente de los spaghetti."
  },
  {
    id: "2",
    formPlato: "Ensalada César con pollo",
    formDuracion: 20,
    formPorciones: 2,
    formImagen: "https://images.pexels.com/photos/1640777/pexels-photo-1640777.jpeg",
    formDificultad: "Baja",
    ingredientes: [
      "2 pechugas de pollo",
      "Lechuga romana",
      "Croutons",
      "Queso parmesano rallado",
      "Aderezo César"
    ],
    pasos: [
      "Cocina las pechugas de pollo y córtalas en tiras.",
      "Lava la lechuga y colócala en un bol.",
      "Agrega el pollo, croutons y queso.",
      "Añade el aderezo al gusto y mezcla bien."
    ],
    formTip: "Puedes añadir un huevo pochado para hacerla más completa.",
    formDescripcionBreve: "Ensalada fresca con pollo a la plancha y aderezo César.",
    formDescripcionAmplia: "Esta ensalada es una opción ligera y deliciosa. El pollo aporta proteína, el queso le da un toque salado y los croutons agregan textura crujiente. Perfecta para el almuerzo."
  },
  {
    id: "3",
    formPlato: "Tacos de carne asada",
    formDuracion: 35,
    formPorciones: 3,
    formImagen: "https://images.pexels.com/photos/461198/pexels-photo-461198.jpeg",
    formDificultad: "Media",
    ingredientes: [
      "500 g de carne de res",
      "Tortillas de maíz",
      "Cilantro fresco",
      "Cebolla picada",
      "Salsa al gusto",
      "Limón"
    ],
    pasos: [
      "Asa la carne con sal y pimienta.",
      "Corta la carne en tiras delgadas.",
      "Calienta las tortillas.",
      "Coloca la carne, cebolla y cilantro en las tortillas.",
      "Agrega salsa y unas gotas de limón."
    ],
    formTip: "Marina la carne con jugo de naranja y ajo para más sabor.",
    formDescripcionBreve: "Tacos tradicionales mexicanos con carne a la parrilla.",
    formDescripcionAmplia: "Estos tacos son un clásico de la comida callejera mexicana. Jugosos, sabrosos y fáciles de preparar en casa para cualquier ocasión."
  },
  {
    id: "4",
    formPlato: "Pizza margarita casera",
    formDuracion: 60,
    formPorciones: 4,
    formImagen: "https://images.pexels.com/photos/825661/pexels-photo-825661.jpeg",
    formDificultad: "Alta",
    ingredientes: [
      "500 g de masa para pizza",
      "200 g de salsa de tomate",
      "250 g de mozzarella",
      "Hojas de albahaca",
      "Aceite de oliva"
    ],
    pasos: [
      "Estira la masa de pizza sobre una bandeja.",
      "Agrega la salsa de tomate.",
      "Distribuye la mozzarella en rodajas.",
      "Hornea a 220°C durante 15-20 minutos.",
      "Decora con albahaca y un chorrito de aceite de oliva."
    ],
    formTip: "Usa mozzarella fresca para un sabor auténtico.",
    formDescripcionBreve: "Pizza italiana con salsa de tomate, queso y albahaca.",
    formDescripcionAmplia: "La pizza margarita es un emblema de la cocina napolitana. Con pocos ingredientes, resalta por su sabor auténtico y su sencillez."
  },
  {
  id: "5",
  formPlato: "Crema de Calabaza",
  formDuracion: 35,
  formPorciones: 4,
  formImagen: "https://images.pexels.com/photos/5421522/pexels-photo-5421522.jpeg",
  formDificultad: "Baja",
  ingredientes: [
    "1 kg de calabaza",
    "1 cebolla",
    "2 dientes de ajo",
    "500 ml de caldo de verduras",
    "2 cucharadas de crema de leche",
    "Aceite de oliva, sal y pimienta"
  ],
  pasos: [
    "Pelar y cortar la calabaza en cubos.",
    "En una olla, sofreír la cebolla y el ajo en aceite de oliva.",
    "Agregar la calabaza y cocinar por unos minutos.",
    "Verter el caldo de verduras y cocinar hasta que la calabaza esté tierna.",
    "Licuar hasta obtener una crema suave.",
    "Agregar la crema de leche, salpimentar al gusto y servir caliente."
  ],
  formTip: "Podés decorar con semillas de calabaza tostadas o un chorrito de aceite de oliva.",
  formDescripcionBreve: "Una sopa cremosa, reconfortante y fácil de preparar.",
  formDescripcionAmplia: "La crema de calabaza es ideal para los días fríos. Su textura suave y su sabor dulce y salado la hacen perfecta como entrada o plato principal. Además, es muy saludable y apta para vegetarianos si omitís la crema de leche o la reemplazás por una opción vegetal."
},
  {
    id: "6",
    formPlato: "Pollo al curry con arroz basmati",
    formDuracion: 50,
    formPorciones: 4,
    formImagen: "https://images.pexels.com/photos/4439740/pexels-photo-4439740.jpeg",
    formDificultad: "Media",
    ingredientes: [
      "500 g de pechuga de pollo en cubos",
      "1 cebolla grande picada",
      "2 dientes de ajo",
      "1 cucharada de jengibre rallado",
      "2 cucharadas de curry en polvo",
      "400 ml de leche de coco",
      "200 g de arroz basmati",
      "Aceite, sal y pimienta al gusto"
    ],
    pasos: [
      "Cocina el arroz basmati en agua con sal hasta que esté listo. Reserva.",
      "En una sartén, sofríe la cebolla, el ajo y el jengibre.",
      "Agrega el pollo y cocina hasta que se dore.",
      "Añade el curry en polvo y mezcla bien.",
      "Vierte la leche de coco y cocina a fuego medio por 20 minutos.",
      "Sirve el pollo con el arroz."
    ],
    formTip: "Agrega unas hojas de cilantro fresco justo antes de servir para un toque aromático.",
    formDescripcionBreve: "Pollo en salsa de curry cremosa, acompañado de arroz aromático.",
    formDescripcionAmplia: "Este plato combina el sabor exótico del curry con la suavidad de la leche de coco. El arroz basmati aporta textura y aroma, convirtiendo esta receta en una excelente opción para una comida nutritiva y con mucho sabor."
  },
  {
    id: "7",
    formPlato: "Ensalada César con Pollo",
    formDuracion: 25,
    formPorciones: 2,
    formImagen: "https://images.pexels.com/photos/1640770/pexels-photo-1640770.jpeg",
    formDificultad: "Baja",
    ingredientes: [
      "1 pechuga de pollo",
      "1 lechuga romana",
      "Croutones",
      "Queso parmesano rallado",
      "Aderezo César",
      "Aceite de oliva",
      "Sal y pimienta"
    ],
    pasos: [
      "Cocinar la pechuga de pollo a la plancha con sal y pimienta. Cortar en tiras.",
      "Lavar y cortar la lechuga en trozos medianos.",
      "En un bowl, mezclar la lechuga con el pollo, croutones y queso.",
      "Agregar aderezo César al gusto y mezclar bien."
    ],
    formTip: "Puedes añadir un huevo cocido para más proteína.",
    formDescripcionBreve: "Ensalada fresca y completa con pollo, parmesano y aderezo César.",
    formDescripcionAmplia: "Una receta ligera, ideal para almuerzos rápidos pero nutritivos. La pechuga de pollo aporta proteína mientras que los croutones y el aderezo César dan textura y sabor. Una comida equilibrada y deliciosa en menos de 30 minutos."
  },
  {
    id: "8",
    formPlato: "Panqueques de Avena y Banana",
    formDuracion: 20,
    formPorciones: 2,
    formImagen: "https://images.pexels.com/photos/376464/pexels-photo-376464.jpeg",
    formDificultad: "Baja",
    ingredientes: [
      "1 banana madura",
      "1 huevo",
      "1/2 taza de avena",
      "1/4 taza de leche",
      "1 cucharadita de polvo de hornear",
      "Aceite para cocinar"
    ],
    pasos: [
      "Tritura la banana en un bowl.",
      "Agrega el huevo, avena, leche y polvo de hornear. Mezcla bien.",
      "Calienta una sartén con un poco de aceite y vierte porciones de mezcla.",
      "Cocina 2-3 minutos por lado hasta que estén dorados."
    ],
    formTip: "Puedes acompañarlos con miel, frutas o yogur.",
    formDescripcionBreve: "Panqueques nutritivos y sin azúcar, perfectos para el desayuno.",
    formDescripcionAmplia: "Estos panqueques son una alternativa saludable y deliciosa para comenzar el día. Sin harina ni azúcar, se preparan en minutos y son ideales para quienes buscan un desayuno balanceado, energético y sabroso."
  }
]